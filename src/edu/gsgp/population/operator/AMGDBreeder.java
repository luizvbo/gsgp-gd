 ...
 */
public class AMGDBreeder extends MGDBreeder{
    protected enum GDTypes{
        ADDITIVE, MULTIPLICATIVE
    }    
   
    public AMGDBreeder(PropertiesManager properties, Double probability) {
        super(properties, probability);
    }
    
    @Override
    public Breeder softClone(PropertiesManager properties) {
        return new AMGDBreeder(properties, this.probability);
    }


    @Override
    public Individual generateIndividual(MersenneTwister rndGenerator, ExperimentalData expData, GSGPIndividual p) {
        if(rndGenerator.nextFloat() < 0.5)
            return generateAGDIndividual(rndGenerator, expData, p);
        return generateMGDIndividual(rndGenerator, expData, p);
    }
    
    private Individual generateAGDIndividual(MersenneTwister rndGenerator, ExperimentalData expData, GSGPIndividual p){
        ...
        double alpha = getAlpha(bounds, numUpperBounds, rndGenerator);
        
        Fitness fitnessFunction = evaluate(p, alpha, expData, GDTypes.ADDITIVE);
    ...
    private Individual generateMGDIndividual(MersenneTwister rndGenerator, ExperimentalData expData, GSGPIndividual p) {
        ...
        Fitness fitnessFunction = evaluate(p, alpha, expData, GDTypes.MULTIPLICATIVE);
        BigInteger numNodes = p.getNumNodes().add(new BigInteger(""+2));
        GSGPIndividual offspring = new GSGPIndividual(numNodes, fitnessFunction);
        return offspring;
    }
    
    private Fitness evaluate(GSGPIndividual ind, double alpha, ExperimentalData expData, GDTypes type){
    ...
    private double getAlpha(ArrayList<Bound> bounds, int numUpperBounds, MersenneTwister rndGenerator){
